...
router.get("/browse", async (req, res) => {
	try {
		const surgeries = await Surgery.find({}).populate(
			"surgeryTeam.doctorId"
		);
...
router.get("/get-patient-notes/:surgeryId/:patientId", grantAccess() , async (req, res) => {
  const surgeries = await Surgery.find({}).select('surgeryTitle')
  const currentSurgeryDetail = await Surgery.findById(req.params.surgeryId)
  .populate({
    path: 'surgeryTeam.doctorId',
    model: 'Doctor',
  })
